...
func TestDivide(t *testing.T) {
	tables := []struct {
		...
		{0, 2.0, 0, ""},
		{-4, 2.0, -2.0, ""},
		{2.0, 0, 0, "cannot divide by 0"}, // comment to decrease cover
	}
...
func TestModule(t *testing.T) {
	...
	for _, table := range tables {
		if total, err := Module(table.x, table.y); err != nil && err.Error() != table.e {
			t.Errorf("Module of (%d*%d) produced wrong error, got %v, want %v.", table.x, table.y, err.Error(), table.e)
		} else if total != table.n {
			t.Errorf("Module of (%d*%d) produced wrong result, got %d, want %d.", table.x, table.y, total, table.n)
		}
	}
}
...
func TestOr(t *testing.T) {
	tables := []struct {
		...
		for _, table := range tables {
			if total, err := Or(table.x, table.y); err != nil && err.Error() != table.e {
				t.Errorf("Or of (%d*%d) produced wrong error, got %v, want %v.", table.x, table.y, err.Error(), table.e)
			} else if total != table.n {
				t.Errorf("Or of (%d*%d) produced wrong result, got %d, want %d.", table.x, table.y, total, table.n)
			}
		}
}
...
func TestAnd(t *testing.T) {
	tables := []struct {
		...
		for _, table := range tables {
			if total, err := And(table.x, table.y); err != nil && err.Error() != table.e {
				t.Errorf("And of (%d*%d) produced wrong error, got %v, want %v.", table.x, table.y, err.Error(), table.e)
			} else if total != table.n {
				t.Errorf("And of (%d*%d) produced wrong result, got %d, want %d.", table.x, table.y, total, table.n)
			}
		}
}